...
export function useChatAi(options = {}) {
  ...
  async function submitFeedback(messageId, rating) {
    ...
    if (!conversationId || !target || feedbackProcessing.value) return false
    feedbackProcessing.value = messageId
    try {
      if (target.feedback?.rating === rating) {
        await deleteMessageFeedback(conversationId, messageId)
        target.feedback = null
      } else {
        target.feedback = await saveMessageFeedback(conversationId, messageId, { rating })
      }
      return true
    } catch (error) {
      emitError(error)
