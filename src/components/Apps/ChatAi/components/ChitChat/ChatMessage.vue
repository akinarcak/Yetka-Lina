    ...
    <div class="chat-message__main">
      ...
      >
        ...
        <button
          ...
          :class="{ 'is-active': message.feedback?.rating === 'positive' }"
          type="button"
          :aria-label="t('ChatAIHelpful')"
          :disabled="feedbackProcessing"
          :title="t('ChatAIHelpful')"
          @click="emit('feedback', message.id, 'positive')"
        >
          <svg class="feedback-icon" viewBox="0 0 24 24" aria-hidden="true">
            <path
              d="M7 10v12M15 5.9 14 10h5.8a2 2 0 0 1 1.9 2.6l-2.3 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.8a2 2 0 0 0 1.8-1.1L12 2a3.1 3.1 0 0 1 3 3.9Z"
            />
          </svg>
        </button>
        <button
          v-if="message.role === 'assistant' && message.status === 'completed'"
          :class="{ 'is-active': message.feedback?.rating === 'negative' }"
          type="button"
          :aria-label="t('ChatAINotHelpful')"
          :disabled="feedbackProcessing"
          :title="t('ChatAINotHelpful')"
          @click="emit('feedback', message.id, 'negative')"
        >
          <svg class="feedback-icon is-down" viewBox="0 0 24 24" aria-hidden="true">
            <path
              d="M7 10v12M15 5.9 14 10h5.8a2 2 0 0 1 1.9 2.6l-2.3 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.8a2 2 0 0 0 1.8-1.1L12 2a3.1 3.1 0 0 1 3 3.9Z"
            />
          </svg>
